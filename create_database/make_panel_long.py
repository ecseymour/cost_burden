...
for y in years:
	print y
	cols = ['NHGISJOIN', 'PLACE', 'STATE', 'pop{}'.format(y), 'rent{}'.format(y), 'inc{}'.format(y), 'burd{}'.format(y), 'vac{}'.format(y), 'renterHH{}'.format(y), 'renterPct{}'.format(y)]
	if y=='80':
		final_df = df[cols].copy(deep=True)
		final_df.columns = ['NHGISJOIN', 'PLACE', 'STATE', 'pop', 'rent', 'inc', 'burd', 'vac', 'renterHH', 'renterPct']
		final_df['year'] = '1980'
		final_df.index = final_df['NHGISJOIN'] + final_df['year']
		print final_df.head()
	else:
		temp = df[cols].copy(deep=True)
		temp.columns = ['NHGISJOIN', 'PLACE', 'STATE', 'pop', 'rent', 'inc', 'burd', 'vac', 'renterHH', 'renterPct']
